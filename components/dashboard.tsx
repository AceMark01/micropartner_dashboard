...
import { fetchDashboardData, fetchIndirectSaleData, type MicropartnerData } from "@/lib/google-sheets"
...
export function Dashboard({ onLogout, user }: DashboardProps) {
  ...
  const [indirectData, setIndirectData] = useState<MicropartnerData[]>([])
  ...
  useEffect(() => {
    ...
    // Fetch Indirect Sale Data
    fetchIndirectSaleData().then((fetchedData) => {
        setIndirectData(fetchedData)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50 flex flex-col">
      ...
      <main className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-8 flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full max-w-[400px] grid-cols-2 bg-white/50 backdrop-blur-xl border border-white/50 p-1.5 rounded-full h-auto mx-auto shadow-sm relative z-10">
            <TabsTrigger 
              value="direct" 
              className="rounded-full data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-600 data-[state=active]:to-fuchsia-600 data-[state=active]:text-white data-[state=active]:shadow-md text-slate-600 font-medium py-2.5 transition-all duration-300 data-[state=active]:scale-[1.02]"
            >
              Direct Sale
            </TabsTrigger>
            <TabsTrigger 
          ...
          <AnimatePresence mode="wait">
            {activeTab === "direct" ? (
              <TabsContent value="direct" className="mt-0" forceMount>
                 <motion.div
                    key="direct"
                 ...
                 >
                    <DashboardTabContent data={data} loading={loading} user={user} />
                 </motion.div>
              </TabsContent>
            ) : (
                <TabsContent value="indirect" className="mt-0" forceMount>
                 <motion.div
                    key="indirect"
                    initial={{ opacity: 0, x: 50 }}
                    animate={{ opacity: 1, x: 0 }}
                    ...
                    transition={{ duration: 0.4, ease: "easeOut" }}
                 >
                    <DashboardTabContent data={indirectData} loading={indirectLoading} user={user} />
                 </motion.div>
              </TabsContent>
            )}
          </AnimatePresence>
...
function DashboardTabContent({ data, loading, user }: DashboardTabContentProps) {
